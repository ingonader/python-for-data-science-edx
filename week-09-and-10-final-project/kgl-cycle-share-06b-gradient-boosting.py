...
var_imp = pd.DataFrame(
    ...
    'importance' : list(mod_gb.feature_importances_)})
...
pdp_current = pdp.pdp_isolate(
    model = mod_gb, dataset = dat_train_x.join(dat_train_y), 
    num_grid_points = 20, grid_type = 'percentile', ##default: 'percentile'
    n_jobs = 1, ## needs to be 1 for XGBoost model!
...
inter_current = pdp.pdp_interact(
    model = mod_gb, dataset = dat_train_x.join(dat_train_y),
    num_grid_points = [10, 10], n_jobs = 1, ## needs to be 1 for XGBoost model!
